...
condvar_t *condvar_create(condvar_type_t type)
{
	switch (type)
	{
		...
		default:
		{
			...
			this->public.timed_wait = (int(*)(condvar_t*, mutex_t *mutex, u_int timeout))timed_wait;
			this->public.timed_wait_abs = (int(*)(condvar_t*, mutex_t *mutex, timeval_t time))timed_wait_abs;
